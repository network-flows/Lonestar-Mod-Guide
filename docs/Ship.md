...
mod飞船默认**不兼容**游戏本体的内容，如果你想让飞船兼容本体或其他模组的天赋，宝物，驾驶员，事件等物品，可以在所有mod加载完毕后，把它们批量迁移到你的飞船。

注意！如果你的飞船部件池为空，则战斗结束后不会出现部件奖励；如果事件池为空，则度假不会出现。这些功能在迁移后将恢复正常。

迁移方法请参考`TutorialMod.cs`中的`MigrateItems`函数。（在此之前请先阅读[补丁教程](Patch.md)）

你可以选择迁移一部分宝物（例如只迁移护盾号的部件池）；另外迁移不是必需的：如果你的飞船有一整套自己专属的部件，宝物，事件等，也可以不迁移。
